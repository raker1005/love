...
function buildAlarmCardHTML(alarm) {
  ...
  // 파트너가 나를 위해 추가한 알람 배지
  const partnerAddedBadge = alarm.isPartnerAdded
    ? `<div class="alarm-partner-badge"><i class="fas fa-heart"></i> ${alarm.createdByNickname || '파트너'}가 추가한 알람</div>`
    : '';

  // 파트너 알람음 배지
  const partnerSoundBadge = alarm.partnerSoundUri
    ? `<div class="alarm-partner-badge"><i class="fas fa-music"></i> 파트너 알람음</div>`
    : '';
...
function deleteAlarm(id) {
  ...
  const alarm = State.alarms[idx];
  State.alarms.splice(idx, 1);
  saveAlarms();
  scheduleAlarms();

  // 파트너가 추가한 알람이면 Firestore에서도 직접 삭제
  if (alarm.isPartnerAdded) {
    const fb = getFirebase();
    if (fb && State.user) {
      const { db, doc, deleteDoc } = fb;
      deleteDoc(doc(db, 'users', State.user.uid, 'shared_alarms', id))
        .catch(e => console.warn('파트너 추가 알람 Firestore 삭제 실패:', e));
    }
  } else {
    syncMyAlarmsToFirebase();
  }
...
function initAuthListener() {
  ...
  fb.onAuthStateChanged(fb.auth, async (user) => {
    ...
    if (user) {
      ...
      subscribeToMySharedAlarms();  // ← 파트너가 추가한 알람 수신
    } else {
      State.myProfile = null;
      State.partnerProfile = null;
      State.partnerAlarms = [];
...
// ============================================================
//  내 shared_alarms 실시간 리스너
//  → 파트너가 내 폰에 추가한 알람(isPartnerAdded=true)을
//    내 로컬 알람 목록에 자동 반영
// ============================================================
function subscribeToMySharedAlarms() {
  const fb = getFirebase();
  if (!fb || !State.user) return;
  const { db, collection, onSnapshot } = fb;
  ...
  const unsub = onSnapshot(col, snap => {
    ...
    snap.docChanges().forEach(change => {
      ...
      // 파트너가 추가해준 알람만 처리 (내가 직접 만든 것은 제외)
      if (!data.isPartnerAdded) return;

      const alarmId = data.alarmId;

      if (change.type === 'added') {
        // 이미 로컬에 있으면 스킵
        if (State.alarms.find(a => a.id === alarmId)) return;
        ...
        const newAlarm = {
          ...
          partnerSoundUri: null,
          soundUri: 'default',
          isPartnerAdded: true,            // 파트너가 추가한 알람 표시
          createdBy: data.createdBy || '',
          createdByNickname: data.createdByNickname || '파트너',
          createdAt: data.createdAt || Date.now()
        };
        State.alarms.push(newAlarm);
        changed = true;
        showToast(`💕 ${data.createdByNickname || '파트너'}가 알람을 추가했어요!`);
        ...
        if (alarm) {
          ...
          alarm.label = data.label || alarm.label;
          alarm.isEnabled = data.isEnabled !== false;
          alarm.repeatDays = data.repeatDays || alarm.repeatDays;
          changed = true;
        }

      } else if (change.type === 'removed') {
        const idx = State.alarms.findIndex(a => a.id === alarmId && a.isPartnerAdded);
        if (idx !== -1) {
          State.alarms.splice(idx, 1);
          changed = true;
          showToast('파트너가 알람을 삭제했어요');
        }
      }
    });
  ...
  }, err => console.warn('MySharedAlarms listener error', err));
...
async function syncMyAlarmsToFirebase() {
  ...
  try {
    ...
    // isPartnerAdded=true 인 알람(파트너가 추가해준 것)은 절대 삭제하지 않음
    await Promise.all(
      existing.docs
        .filter(d => !d.data().isPartnerAdded)   // 내가 직접 만든 것만 삭제
        .map(d => deleteDoc(d.ref))
    );

    // 내 로컬 알람(isEnabled) 다시 업로드
    const enabledAlarms = State.alarms.filter(a => a.isEnabled && !a.isPartnerAdded);
